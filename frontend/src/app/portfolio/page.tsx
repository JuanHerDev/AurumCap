...
export default function PortfolioPage() {
  const { summary, investments, loading, error, refresh, create, update, remove } = usePortfolio(15000);
  ...
  async function onDelete(item: any) {
    if (!confirm(`Eliminar ${item.symbol}? Esto no se puede deshacer.`)) return;
    try {
      await remove(item.id ?? item.inv_id ?? item._id);
  ...
  return (
    <div style={{padding:20, maxWidth:900, margin:"0 auto", background:"#071026", minHeight:"100vh", color:"#fff"}}>
      ...
      <section style={{display:"grid", gridTemplateColumns:"1fr 320px", gap:16, marginBottom:16}}>
        <div style={{background:"#0b1320", padding:16, borderRadius:12}}>
          ...
          <div style={{height:200, display:"flex", alignItems:"center", justifyContent:"center"}}>
            <img src={"Logo.png"} alt="mock" style={{maxHeight:160, opacity:0.15}}/>
          </div>
        </div>
      ...
      <section>
        {investments?.length ? investments.map((it: any) => (
          <InvestmentCard key={it.id ?? it.symbol} item={it} onEdit={onEdit} onDelete={onDelete} />

...
async def investments_summary(
    ...
    for inv in investments:
        ...
        if inv.asset_type == "crypto":
            price_raw = get_crypto_price(inv.symbol)
        elif inv.asset_type == "stock":
            price_raw = get_stock_price(inv.symbol)
        else:
            price_raw = 0
        ...
        if inv.asset_type == "stock":
            fund_data = get_fundamentals(inv.symbol) or {}
    ...
    def r(x): return x.quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
